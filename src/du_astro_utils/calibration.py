...
def get_infos_from_image(fits_image_path):
    ...
    print(
        f"{acq_type} ({size_x}x{size_y}) taken in band {acq_filter} with {acq_cam} on {telescope} on {epoch} ({expos_time}s exposure)."
    )
...
def load_dark_frames(
    path_to_darks_dir, aq_date, aq_cam, expos_time, size_x, size_y, override_date_check=False
):
    ...
    for _file in os.listdir(_path):
        ...
        _dat, _scope, _cam, _filt, _dur, _x, _y = get_infos_from_image(_fpath)
        if (
            (check_obs_night(_dat, aq_date) or override_date_check)
            and _x == size_x
            and _y == size_y
            and _dur == expos_time
        ):
...
def load_flat_frames(
    path_to_flats_dir, aq_date, aq_cam, aq_filter, size_x, size_y, override_date_check=False
):
...
def master_dark(dark_frames_list, use_bias=False, master_bias=""):
    ...
    print(
        f"Fraction of hot pixels (%) : {100*len(master_dark_as_array[hot_pix_loc])/len(master_dark_as_array.flatten()):.2f}"
    )
...
def master_flat(flat_frames_list, master_dark_path):
    ...
    print(
        f"Fraction of dead pixels (%) : {100*len(master_flat_as_array[dead_pix_loc])/len(master_flat_as_array.flatten()):.2f}"
    )
