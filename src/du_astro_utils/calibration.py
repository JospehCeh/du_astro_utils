...
def load_bias_frames(path_to_bias_dir, aq_date, aq_cam, size_x, size_y, override_date_check=False, max_days=7, verbose=True):
    ...
    for _file in os.listdir(_path):
        ...
        if os.path.isfile(_fpath) and "fits" in ext.lower():
            _dat, _scope, _cam, _filt, _dur, _x, _y = get_infos_from_image(_fpath, verbose=verbose)
...
def load_flat_frames(path_to_flats_dir, aq_date, aq_cam, aq_filter, size_x, size_y, override_date_check=False, max_days=7, verbose=True):
    """
    ...
    for _file in os.listdir(_path):
        ...
        if os.path.isfile(_fpath) and "fits" in ext.lower():
            ...
            if (check_obs_night(_dat, aq_date, max_days, verbose=verbose) or override_date_check) and _x == size_x and _y == size_y and compat_filt:
...
def master_bias(bias_frames_list, overwrite=False, verbose=True):
    ...
    _dat, _scope, camera, _filt, _dur, _x, _y = get_infos_from_image(bias_frames_list[0], verbose=verbose)
    ...
    if make_calib:
        ...
        master_bias_as_array = fits.open(write_path)[0].data
...
def master_dark(dark_frames_list, use_bias=False, master_bias="", overwrite=False, verbose=True):
    ...
    _dat, _scope, camera, _filt, exposure, _x, _y = get_infos_from_image(dark_frames_list[0], verbose=verbose)
    date = _dat.date().isoformat()
    ...
    if make_calib:
        ...
        master_dark_as_array = fits.open(md_write_path)[0].data
        hot_pixels_map = fits.open(hp_write_path)[0].data
...
def master_flat(flat_frames_list, master_dark_path, overwrite=False, verbose=True):
    """
    ...
    _dat, _scope, camera, band, exposure, _x, _y = get_infos_from_image(flat_frames_list[0], verbose=verbose)
    date = _dat.date().isoformat()
    ...
    if make_calib:
        ...
        master_flat_as_array = fits.open(mf_write_path)[0].data
        dead_pixels_map = fits.open(dp_write_path)[0].data
...
def reduce_sci_image(fits_image, path_to_darks_dir, path_to_flats_dir, path_to_bias_dir="", use_bias=False, override_date_check=False, max_days=7, speedup=False, verbose=False, overwrite=False, write_tmp=True):
    ...
    if make_reduced:
        ...
        flats_list = load_flat_frames(path_to_flats_dir, sc_date, sc_cam, sc_filter, sc_x, sc_y, override_date_check=override_date_check, max_days=max_days, verbose=verbose)
        ...
        if use_bias:
            dic_to_return.update({"MASTER BIAS": MASTER_BIAS["path"]})
    else:
        if verbose:
        ...
        with fits.open(write_path) as hudl:
            ...
            if use_bias:
                dic_to_return.update({"MASTER BIAS": hdr.get("MBIAS")})
